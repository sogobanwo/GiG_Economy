...
use stylus_sdk::{
    ...
    storage::{
        StorageAddress, StorageBool, StorageMap, StorageString,
...
impl GigEconomy {
    ...
    fn approve_submission(&mut self, task_id: U256, submission_id: U256) {
        ...
        let completer = self.task_submissions.get(task_id).get(submission_id).submitter.get();
        self.task_submissions.setter(task_id).setter(submission_id).approved.set(true);
        let bounty;
        let token_addr;
        {
...
impl GigEconomy {
    ...
    fn _assert_owner(&self, _address: &Address) {
        assert!(
            *_address == self._get_msg_sender(), "not owner"
        )
    }
}